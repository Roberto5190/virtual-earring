...
const drawPath = (ctx, points, closePath) => {
    const region = new Path2D(); // initialize a new Path2D object
    region.moveTo(points[0][0], points[0][1]); // especify the starting point
    ...
    // If closePath is true, close the path
    if (closePath) {
        region.closePath();
    }
    ctx.strokeStyle = "aqua"; // set the stroke color
    ctx.stroke(region); // stroke the path
};
        ...
        predictions.forEach((prediction) => {
            ...
            for (let i = 0; i < TRIANGULATION.length / 3; i++) {
                const points = [
                    ...
                    TRIANGULATION[i * 3 + 1],
                    TRIANGULATION[i * 3 + 2],
                ].map((index) => keypoints[index]); // get the points from the predictions
                drawPath(ctx, points, true); // draw the path with closePath set to true
            }
...
export const drawEarlobes = (face, canvasRef) => {

    if (face.length > 0) {
        const ctx = canvasRef.current.getContext("2d");
        // Obtener coordenadas 2D de cada lóbulo (izquierdo 234, derecho 454)
        const [xLeft, yLeft] = face[0].scaledMesh[234];
        const [xRight, yRight] = face[0].scaledMesh[454];
        // Dibujar un círculo rojo en el lóbulo izquierdo
        ctx.beginPath();
        ctx.arc(xLeft, yLeft, 5, 0, 2 * Math.PI);
        ctx.fillStyle = "red";
        ctx.fill();
        // Dibujar un círculo rojo en el lóbulo derecho
        ctx.beginPath();
        ctx.arc(xRight, yRight, 5, 0, 2 * Math.PI);
        ctx.fillStyle = "red";
        ctx.fill();
    }
};
